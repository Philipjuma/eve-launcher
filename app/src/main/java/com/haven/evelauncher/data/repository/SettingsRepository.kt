...
class SettingsRepository(context: Context) {
    ...
    private fun loadSettings(): EveSettings {
        return EveSettings(
            clockSize = ClockSize.valueOf(prefs.getString("clock_size", ClockSize.LARGE.name) ?: ClockSize.LARGE.name),
            isClockDynamic = prefs.getBoolean("clock_dynamic", true),
            personalClockColor = prefs.getInt("clock_personal_color", 0xFFFFFFFF.toInt()),
            blurIntensity = BlurIntensity.valueOf(prefs.getString("blur_intensity", BlurIntensity.BALANCED.name) ?: BlurIntensity.BALANCED.name),
